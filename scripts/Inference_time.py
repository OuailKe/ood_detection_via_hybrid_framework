...
OUTPUT_DIR   = PROJECT_ROOT / 'plots' / 'timing'
...
def time_hybrid(model, tensors, grays, class_means, precision,
                tau_benford, tau_mahalanobis, n_warmup, n_measure):
    """
    Time full hybrid pipeline per image:
      Stage 1: Benford χ² check
      Stage 2 (if passes): Mahalanobis distance check
    Reports both worst-case (always run both stages) and
    average-case (early exit after Benford rejection) times.
    """
    features_store = []
    ...
    times_worst  = []   # always run both stages
    times_avg    = []   # early exit after Benford rejection

    with torch.no_grad():
        for t, g in zip(tensors[n_warmup:n_warmup + n_measure],
                        grays[n_warmup:n_warmup + n_measure]):
            # ── Worst case: run both regardless ──────────────────────────────
            torch.cuda.synchronize()
            t0 = time.perf_counter()
            ...
            dists = [feat @ precision @ feat]   # simplified
            for mean in class_means:
                diff = feat - mean
            ...
            torch.cuda.synchronize()
            times_worst.append((time.perf_counter() - t0) * 1000)

            # ── Average case: early exit if Benford rejects ───────────────────
            torch.cuda.synchronize()
            t0   = time.perf_counter()
            chi2 = compute_dct_benford_chi2(g)

            if chi2 <= tau_benford:
                # Passes Stage 1 — run semantic stage
                features_store.clear()
                _ = model(t)
                feat  = features_store[0][0]
                dists = []
            ...
            torch.cuda.synchronize()
            times_avg.append((time.perf_counter() - t0) * 1000)

    hook.remove()
    return times_worst, times_avg
...
def main():
    ...
    print('  → Hybrid (worst + average case)...')
    hybrid_worst, hybrid_avg = time_hybrid(
        model, tensors, grays,
        class_means, precision,
        tau_benford, tau_mahalanobis,
        N_WARMUP, N_MEASURE)
    results.append(summarise(hybrid_worst, 'Hybrid (worst case)'))
    results.append(summarise(hybrid_avg,   'Hybrid (avg case, early exit)'))
