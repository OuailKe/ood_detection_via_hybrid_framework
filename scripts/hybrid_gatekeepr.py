...
OUTPUT_DIR   = PROJECT_ROOT / 'plots' / 'gatekeeper'
...
def combine_benford_and_semantic(
    ...
    """
    Two-stage gatekeeper decision:

      Stage 1: Reject if chi2 > tau_benford
               tau_benford calibrated so that only `benford_fpr_budget`
               fraction of ID samples are falsely rejected by Benford alone.

      Stage 2: Reject if semantic_score < tau_semantic
               (i.e. semantic conf below ID 5th percentile)

      Accept otherwise.

    Returns combined confidence scores where:
      - Samples rejected by either gate get conf = -inf (lowest possible)
      - Samples accepted by both gates keep their semantic conf score

    This lets us reuse the same metric functions on the combined scores.
    """
    # Calibrate Benford threshold on ID set
    # Allow at most benford_fpr_budget of ID images to be rejected
    tau_benford  = np.percentile(id_benford,
                                 (1 - benford_fpr_budget) * 100)

    # Calibrate semantic threshold on ID set
    ...
    def gate(benford_scores, semantic_scores):
        """
        Returns binary accept/reject array.
        True  = REJECT (OOD)
        False = ACCEPT (ID)
        """
        benford_reject = benford_scores > tau_benford

        if semantic_higher_is_id:
            semantic_reject = semantic_scores < tau_semantic
        else:
            semantic_reject = semantic_scores > tau_semantic

        return benford_reject | semantic_reject   # reject if EITHER gate fires

    reject_id  = gate(id_benford,  id_semantic)
    reject_ood = gate(ood_benford, ood_semantic)

    return reject_id, reject_ood, tau_benford, tau_semantic
...
def main():
    ...
    for sem_name in ['msp', 'mahalanobis']:
        ...
        for group_name, dataset_names in OOD_GROUPS.items():
            ...
            reject_id, reject_ood, tau_b, tau_s = combine_benford_and_semantic(
                id_benford, ood_b,
                id_sem,     ood_s,
                benford_fpr_budget=0.05,
                semantic_tpr=0.95,
            ...
            # FPR@95: fraction of OOD samples accepted (not rejected) by gatekeeper
            fpr = (~reject_ood).mean()

            # For AUROC/AUPR: use semantic scores but set rejected samples to
            # worst possible score so they rank as OOD
            # ID: rejected → lowest conf; accepted → original conf
            ...
            # higher conf = more ID → negate for OOD ranking
